...
- **Abonnement : Stripe est branché en production depuis le 2026-09-10, clé live confirmée le 2026-09-11.** L'application se ferme après 7 jours d'essai. Le site ne doit donc **jamais** se dire gratuit : plus aucune « Inscription gratuite », plus aucun `Offer` à `price: "0"`. Les seuls tarifs publiables sont ceux du paywall, lus dans `src/appTranslations.ts` (`planMonthlyPrice` 5,99 €, `planAnnualPrice` 49 €, `planSub` essai 7 jours), et **tout prix déclaré en JSON-LD doit être lisible dans le texte visible de la même page**. Quatre tests de `seo.test.ts` le tiennent, `seo:verify-prod` le revérifie en production. **« Lisible dans le texte visible » veut dire hors JSON-LD** : jusqu'au 2026-09-17, les deux contrôles cherchaient le prix dans le HTML entier et le retrouvaient dans l'`Offer` qu'ils vérifiaient — `/budget-voyage` a publié 5,99 € et 49 € sans les écrire nulle part. Un test de visibilité doit retirer `<script>` et `<style>`, et ignorer le bloc « À lire aussi », qui décrit les pages voisines. Règle générale : **une page qui définit son propre `ctaText` perd le bloc par défaut, seul endroit où le tarif figure** — elle doit alors le redire elle-même. Ne jamais juger l'état de Stripe sur le `.env` **local** : c'est l'erreur du run du 2026-09-11 au matin — la vérité est dans les variables Railway de `backend-api`. **Les pages légales aussi parlent de prix** : le 2026-09-19, les CGV (`src/legalContent.ts:260`, EN l. 573, servies en `/?legal=sales` et indexées par Google) disaient encore « le service est proposé gratuitement et aucune offre payante n'est commercialisée ». Aucun contrôle ne les couvre (vue SPA, pas de HTML statique) : les relire par `grep -n "gratuit\|free" src/legalContent.ts` à chaque run. Texte juridique : la routine signale, Marion décide (backlog `cgv-offre-payante`).
...
`seo/verify-prod.mjs` ne contient **aucune valeur attendue en dur** : il relit `index.html`, `state.json`, `public/robots.txt`, `server.mjs` et `src/appTranslations.ts`, puis vérifie que la production dit la même chose (statuts, H1 unique, title/description/canonical, prix JSON-LD = prix du paywall, prix lisibles, aucune « Inscription gratuite », robots.txt identique et routes privées fermées **dans chaque groupe**, `X-Robots-Tag`, sitemap complet, llms.txt, 404 réel) **et, depuis le 2026-09-14, que la production sert au caractère près ce que le dépôt génère** : HTML de chaque guide = `renderPage()`, `sitemap.xml` et `llms.txt` = `renderSite()`, JSON-LD de l'accueil = celui d'`index.html`, et **depuis le 2026-09-18 le texte visible de l'accueil** (le `<body>` sans scripts, styles ni balises) = celui d'`index.html`. Code de sortie 1 si une vérification échoue (49 contrôles).

**Pourquoi le texte de l'accueil aussi.** Le 2026-09-18, l'accueil en production taisait encore Viator (correction du 16/09 non poussée) et la ligne « / » passait : seuls son head et son JSON-LD étaient comparés, parce que Vite hache les noms d'assets. Le texte du `<body>`, lui, est identique entre `index.html` et `dist/index.html` — c'est donc lui qu'on compare. Tout contenu servi aux robots doit être couvert par une égalité avec le dépôt, jamais par un échantillon.

**Pourquoi l'égalité exacte.** Le 2026-09-14, la production disait encore « l'inscription est gratuite » sur `/planificateur-voyage-ia` (correction du 13/09 non poussée) et le vérificateur affichait 41/41 : il ne cherchait que la chaîne « Inscription gratuite ». Une liste de phrases interdites a toujours un trou ; l'égalité avec le build n'en a pas. Conséquence voulue : **après chaque commit SEO local, le vérificateur échoue tant que Marion n'a pas poussé** — c'est le signal, pas un faux positif. Une ligne « HTML identique au dépôt » en ÉCHEC se lit avec son détail (caractère et extrait prod ≠ dépôt) et `git log origin/main..HEAD`.
...
Puis, une fois la production saine :
...
- **Pages légales `?legal=`** : Google les indexe comme pages distinctes (relevé du 2026-09-18). Elles ne sont pas des pages SEO ; ne pas les « optimiser » — le backlog `canonical-pages-legales` attend une décision de Marion.
- **Règle CTR** (dès que Search Console existe) : requêtes 28 j avec ≥ 50 impressions, CTR < 2 % et position ≤ 15 → la page ciblée reçoit un title (≤ 80 caractères avant « | Mon Petit Voyageur ») et une description (≤ 190 caractères) qui répondent mot pour mot à la requête, avec un bénéfice concret et, si la page est commerciale, l'argument tarifaire exact (« Essai gratuit de 7 jours, puis 5,99 €/mois ») — **jamais** « Inscription gratuite », que les tests interdisent depuis le branchement de Stripe. Une page par run, requête et date consignées dans `state.json → kpi`, relecture 28 j après.
